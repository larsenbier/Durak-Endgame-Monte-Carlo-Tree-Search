...
* **States** We let $\Sigma$ denote the set of possible states. This is simply the set of all possible hands, talons, and discard piles over some set of cards $C$.
* **Sensor Model** For $o\in\Omega$ and $s\in\Sigma$, $\mathbf{P}(o|s)$ is the probability distribution of $\Omega$ given $s$.
* **Actions** We let $A_{s}$ denote the set of available actions in state $s$, or just $A$ when the state is clear. The available actions are specified in the rules for Perevodnoy Durak linked in the first paragraph.
